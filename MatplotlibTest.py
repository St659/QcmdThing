...
class MyStaticMplCanvas(MyMplCanvas):
    ...
    def compute_initial_figure(self):
        ...
        self.points_t = np.column_stack((t))
        self.points_s = np.column_stack((s))

    def onclick(self, event):
        print("Press set")
        self.press = event.button, event.x, event.y, event.xdata, event.ydata
    ...
    def onmove(self, event):
        if self.press is None:
           ...
           if event.x > self.prevX:
                idx = np.nanargmin(((self.points_t - (event.xdata)) ** 2))
           ...
           elif  event.x < self.prevX and event.x > self.initialx:
               try:
                    self.xdata.pop()
                    self.ydata.pop()
               except IndexError:
